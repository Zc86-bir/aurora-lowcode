...
- [十五、PHASE 11 Supervisor Foundation](#十五phase-11-supervisor-foundation)
- [十六、DEV 启动与联调稳定化（2026-05）](#十六dev-启动与联调稳定化2026-05)
...
| **前端 CDN 缓存** | `FrontendCacheConfig`（HTML no-cache + assets immutable 365d）、Caddy 模板 + Helm Ingress CDN 注解 |
...
| **Promtail 日志采集** | Docker/K8s/Caddy 容器日志（JSON 格式）+ traceId/tenantId/level 标签提取 |
...
### 2.12 PHASE 11 — Supervisor Foundation ✅

| 类别 | 成果 |
|------|------|
| **Supervisor DTO 边界** | `SupervisorPlan`、`SupervisorSubTask`、`SupervisorRequest`、`SupervisorResult`、`RollbackEntry`、`ImmutablePayloads`，提供 plan 结构校验与 payload 不可变冻结 |
| **DAG 校验** | `DagValidator` + `DagValidationResult`，支持 missing dependency / duplicate dependency / self-dependency / cycle 检测，以及稳定拓扑序 |
| **Prompt Contract** | `SupervisorPromptFactory` 产出 strict JSON-only contract，约束 `planName/tasks/taskId/skillId/dependencies/parameters` |
| **批调度与回滚** | `DefaultSupervisorOrchestrator` 基于 `StructuredTaskScope` 执行 dependency batch，支持 fail-fast、timeout materialization、reverse-order rollback |
| **API 接入** | `ApiGatewayController` 暴露 `/api/v1/generate/app`，把复合应用 prompt 委派给 `SupervisorOrchestrator` |
| **测试覆盖** | `SupervisorDtoShapeTest`、`DagValidatorTest`、`SupervisorPromptFactoryTest`、`DefaultSupervisorOrchestratorTest`、`ApiGatewayControllerSupervisorTest` |

### 2.13 DEV 启动与联调稳定化（2026-05） ✅

| 类别 | 成果 |
|------|------|
| **Redis 冲突排障** | 明确 dev 需连接 Docker Redis（`aurora_redis_password`），避免本机无密码 Redis 服务占用 `6379` 导致 Redisson `AUTH` 失败。 |
| **AI 自动配置隔离** | dev profile 排除 `AnthropicAutoConfiguration` 与 `OpenAiAutoConfiguration`，避免无 API Key 阻断应用启动。 |
| **健康检查降噪** | dev profile 关闭 `management.health.mail`，移除本地无邮箱凭证造成的噪声告警。 |
| **API 路由策略** | `ApiGatewayController` 保持 `@Profile("!dev")`，避免 dev 环境因运行时重依赖缺失导致上下文启动失败。 |
| **dev 轻量健康端点** | 新增 `DevApiHealthController`（`/api/v1/health`，仅 `dev`），用于前后端联调最小可用探针。 |
| **鉴权调试开关** | `SecurityFilterChainConfig` 增加 `aurora.security.dev-permit-api`（dev 默认 true）用于联调排障，不影响 prod。 |
| **I18N 403 根因修复** | `I18nController` 使用 `@PathVariable("locale")`，修复参数名缺失时的 `IllegalArgumentException` 与后续 `/error` 403。 |
...
[ApiGatewayController]
        │ 读取 X-Tenant-Id / X-User-Id / prompt
        ▼
[SupervisorOrchestrator.execute()]
        │
        ├── [SupervisorPromptFactory]
        │       └── 约束严格 JSON 计划输出
        │
        ├── [SupervisorPlan / ImmutablePayloads]
        │       └── DTO 边界校验 + payload 冻结
        │
        ├── [DagValidator]
        │       └── 缺失依赖 / 自依赖 / 环检测 + stable topo order
        │
        ├── [Batch Scheduler]
        │       └── 按 dependency depth 分批执行
        │
        ├── [StructuredTaskScope]
        │       └── 同批并行执行 SkillExecutor 子任务
        │
        ├── 失败/超时？ ──是──► [Materialize failure result]
        │                       └── [MetadataHotReloadManager.rollback(...)]
        │                               逆完成顺序补偿
        │
        └── 否
                ▼
...
设计要点：
- 同一 batch 只运行依赖已满足的节点，不允许在 scope 内等待 sibling future。
- batch 内任一任务失败后停止后续 DAG 推进，并对已完成成功结果执行补偿回滚。
- timeout、exception、failed result 都必须 materialize 到最终 `taskResultsByTaskId`，避免仅返回批级错误。
- rollback 资格覆盖 `SkillResult.Success` 与 `SkillResult.FallbackApplied`。

**关键文件**：`DefaultSupervisorOrchestrator.java`、`DagValidator.java`、`SupervisorPromptFactory.java`
...
| 严重级别 | 修复项 |
|----------|--------|
| CRITICAL | SQL 注入修复 — `TenantAwareVectorStore` 改用 PostgreSQL `ANY(?)` 数组参数化 |
| CRITICAL | DoS 防护 — 上传文件限制 50MB |
| CRITICAL | SSRF 防护 — `import-url` 端点复用 `UrlValidator` |
| HIGH | Spring AOP — `@Transactional` 方法改为 public |
| HIGH | 租户过滤 — `getRemotesForTenant(tenantId)` 按 allowedTenants 过滤 |
| HIGH | 输入校验 — knowledgeScope 校验 TENANT/PROJECT/MODULE |
...
| Caddy 配置 | 1（deploy/caddy-local.Caddyfile） |
| Prompt 模板 | 2 |
| ADR 文档 | 4 |
| 审查报告 | 14（含 PHASE 9 + PHASE 10 审查修复） |
| **总计** | **~190** |
### 代码统计
